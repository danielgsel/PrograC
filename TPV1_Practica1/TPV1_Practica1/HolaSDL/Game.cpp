...
Game::Game() {
	...
	*/
		...
		for (int i = 0; i < numBalloons; i++) {
			ballons.push_back(new Balloon( textures[5], this, i,winWidth,winHeight));
		}
		marcador = new ScoreBoard(textures[6], textures[4], numArrows);
...
void Game::generateBalloons(vector<Balloon*>* ball, int i) {
			delete ball->at(i);
			ball->at(i) = new Balloon( textures[5], this, i,winWidth,winHeight);
}
...
bool Game::arrowHitsBalloon(SDL_Rect baloon) {
	int i = 0;
	bool hit = false;
...
void Game::destroyBaloon(int pos) {
	...
	ballons.at(pos) = new Balloon( textures[5],  this, pos,winWidth,winHeight);
	puntuacion++;
